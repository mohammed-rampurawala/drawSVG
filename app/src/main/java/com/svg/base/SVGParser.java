...
import com.svg.base.CSSParser.MediaType;
import com.svg.base.SVG.Box;
import com.svg.base.SVG.CSSClipRect;
...
import com.svg.base.SVG.TextChild;
import com.svg.base.SVG.TextPositionedContainer;
import com.svg.base.SVG.TextRoot;
import com.svg.base.SVG.Unit;
import com.svg.base.SVG.Style.TextDecoration;
import com.svg.base.SVG.Style.TextDirection;
...

{
	...
	private enum SVGElem {
		...
		public static SVGElem fromString(String str) {
			...
			if (elem != null)
				return elem;
			// Manual check for "switch" which is in upper case because it's a
			// Java reserved identifier
	...
	// private static final String TAG_FECOMPONENTTRANSFER =
	// "feComponentTransfer";
	...
	// private static final String TAG_FESPECULARLIGHTING =
	// "feSpecularLighting";
	...
	private enum SVGAttr {
		CLASS, // Upper case because 'class' is a reserved word. Handled as a
				// special case.
		clip, clip_path, clipPathUnits, clip_rule, color, cx, cy, direction, dx, dy, fx, fy, d, display, fill, fill_rule, fill_opacity, font, font_family, font_size, font_weight, font_style,
		...
		public static SVGAttr fromString(String str) {
			...
			// Check for underscore in attribute - it could potentially confuse
			// us
			if (str.indexOf('_') != -1) {
				cache.put(str, UNSUPPORTED);
				return UNSUPPORTED;
	...
	public void startElement(String uri, String localName, String qName,
		...
		if (ignoring) {
			ignoreDepth++;
			// PRinting to see which method is running first
			Log.d(TAG, "Method Ran");
		...
		switch (elem) {
		case svg:
			Log.d("SVG ATTRIBUTE", "SVG");

			// svg(attributes);
			debug("<svg>");

			SVG.Svg objsvg = new SVG.Svg();
			objsvg.document = svgDocument;
			objsvg.parent = currentElement;
			parseAttributesCore(objsvg, attributes);
			parseAttributesStyle(objsvg, attributes);
			parseAttributesConditional(objsvg, attributes);
			parseAttributesViewBox(objsvg, attributes);
			parseAttributesSVG(objsvg, attributes);
			//
			// float width = objsvg.getWidth();
			// if(width != 0.0f){
			// Log.w("widthobj", String.valueOf(width));
			// }
			//
			// float height = objsvg.getHeight();
			// if(height != 0.0f)
			// {
			// Log.w("heightobj", String.valueOf(height));
			// }

			if (currentElement == null) {
				svgDocument.setRootElement(objsvg);
			} else {
				currentElement.addChild(objsvg);
			}

			currentElement = objsvg;
			break;
		case g:
		case a:
			// <a> treated like a group element
			Log.d("G,A ATTRIBUTE", "G A");

			if (currentElement == null)
				throw new SAXException(
						"Invalid document. Root element must be <svg>");
			SVG.Group objg = new SVG.Group();
			objg.document = svgDocument;
			objg.parent = currentElement;
			parseAttributesCore(objg, attributes);
			parseAttributesStyle(objg, attributes);
			parseAttributesTransform(objg, attributes);
			parseAttributesConditional(objg, attributes);

			currentElement.addChild(objg);

			Style style = objg.style;
			// Log.d("style", style+"");
			...
			break;
		case defs:
			Log.d("DEFS ATTRIBUTE", "DEFS");

			defs(attributes);
			break;
		case use:
			Log.d("USE ATTRIBUTE", "USE");

			use(attributes);
			break;
		case path:
			Log.d("PathAttribut", "Path");

			path(attributes); // Cursor is Sticking Here
			break;
		case rect:
			Log.d("RECT ATTRIBUTE", "RECT");

			rect(attributes);
			break;
		case circle:
			Log.d("CIRCLE ATTRIBUTE", "CIRCLE");

			circle(attributes);
			break;
		case ellipse:
			Log.d("ELLIPSE ATTRIBUTE", "ELLIPSE");

			ellipse(attributes);
			break;
		case line:
			Log.d("LINE ATTRIBUTE", "ELLIPSE");

			line(attributes);
			break;
		case polyline:
			Log.d("POLYLINE ATTRIBUTE", "PolyLine");

			polyline(attributes);
			break;
		case polygon:
			Log.d("POLYGON ATTRIBUTE", "Polygon");

			polygon(attributes);
			break;
		case text:
			Log.d("TEXT ATTRIBUTE", "Text");

			text(attributes);
			break;
		case tspan:
			Log.d("TSPAN ATTRIBUTE", "TSPAN");

			tspan(attributes);
			break;
		case tref:
			Log.d("TREF ATTRIBUTE", "TREF ");

			tref(attributes);
			break;
		case SWITCH:
			Log.d("SWITCH ATTRIBUTE", "SWITCH ");

			zwitch(attributes);
			break;
		case symbol:
			Log.d("SYMBOL ATTRIBUTE", "SYMBOL ");

			symbol(attributes);
			break;
		case marker:
			Log.d("MARKER ATTRIBUTE", "MARKER");

			marker(attributes);
			break;
		case linearGradient:
			Log.d("LINEAR GRADIENT ATTRIBUTE", "LINEAR GRADIENT");

			linearGradient(attributes);
			break;
		case radialGradient:
			Log.d("RADIAL GRADIENT ATTRIBUTE", "RADIAL GRADIENT");

			radialGradient(attributes);
			break;
		case stop:
			Log.d("STOP ATTRIBUTE", "STOP GRADIENT");
		...
		case desc:
			...
			break;
		case clipPath:
			Log.d("CLIP PATH", "CLIP PATH");

			clipPath(attributes);
			break;
		case textPath:
			Log.d("TEXT PATH", "TEXT PATH");

			textPath(attributes);
			break;
		case pattern:
			Log.d("PATTERN", "PATTER");
	...
	public void characters(char[] ch, int start, int length)
		...
		if (currentElement instanceof SVG.TextContainer) {
			// The SAX parser can pass us several text nodes in a row. If this
			// happens, we
			...
			if (previousSibling instanceof SVG.TextSequence) {
				...
				((SVG.TextSequence) previousSibling).text += new String(ch,
						start, length);
			} else {
	...
	public void comment(char[] ch, int start, int length) throws SAXException {
		...
		// It is legal for the contents of the <style> element to be enclosed in
		// XML comments (ie. "<!--" and "-->").
		// So we need to include the contents of the comment in the text sent to
		// the CSS parser.
		if (inStyleElement) {
			if (styleElementContents == null)
				styleElementContents = new StringBuilder(length);
	...
	private void parseAttributesSVG(SVG.Svg obj, Attributes attributes)
			throws SAXException {
		for (int i = 0; i < attributes.getLength(); i++) {
			...
			case y:
				...
				break;
			case width:
				// Log.w("WidthValue",val);
				obj.width = parseLength(val);
				if (obj.width.isNegative())
					throw new SAXException(
							"Invalid <svg> element. width cannot be negative");
				break;
			case height:
				// Log.w("HeightValue",val);
				obj.height = parseLength(val);
				if (obj.height.isNegative())
					throw new SAXException(
							"Invalid <svg> element. height cannot be negative");
				break;
			case version:
				// Log.d("Version",val);
				obj.version = val;
	...
	private void parseAttributesUse(SVG.Use obj, Attributes attributes)
			throws SAXException {
		for (int i = 0; i < attributes.getLength(); i++) {
			String val = attributes.getValue(i).trim();
			switch (SVGAttr.fromString(attributes.getLocalName(i))) {
	...
	private void path(Attributes attributes) throws SAXException {
		...
		Log.d("Attributes", String.valueOf(attributes));
		...
		/*
		 * Extracting the PathDefinition Object from the static class Path by
		 * initializing a getter of PathDefinition Object
		 */
		PathDefinition d = obj.getD();

		Log.w("Running d", "running d");

		/*
		 * As now i have the object of PathDefinition and I can access the
		 * values so also defined 2 getters of coords and commands List to get
		 * the coords and commands of the list and adding it to the 2 static
		 * List Objects defined in MainActivity class.
		 */
		// List<Byte> commands = d.getCommands();

		// MainActivity.commands.addAll(commands);
		Log.w("Running d", "running d");

		// List<Float> coords = d.getCoords();

		// MainActivity.coords.addAll(coords);
		// Log.w("Running d","running d");

		currentElement.addChild(obj);
	}

	private void parseAttributesPath(SVG.Path obj, Attributes attributes)
			throws SAXException {
		for (int i = 0; i < attributes.getLength(); i++) {
			String val = attributes.getValue(i).trim();
			switch (SVGAttr.fromString(attributes.getLocalName(i))) {
	...
	private void parseAttributesRect(SVG.Rect obj, Attributes attributes)
			throws SAXException {
		for (int i = 0; i < attributes.getLength(); i++) {
			String val = attributes.getValue(i).trim();
			switch (SVGAttr.fromString(attributes.getLocalName(i))) {
	...
	private void parseAttributesCircle(SVG.Circle obj, Attributes attributes)
			throws SAXException {
		for (int i = 0; i < attributes.getLength(); i++) {
			String val = attributes.getValue(i).trim();
			switch (SVGAttr.fromString(attributes.getLocalName(i))) {
	...
	private void parseAttributesLine(SVG.Line obj, Attributes attributes)
			throws SAXException {
		for (int i = 0; i < attributes.getLength(); i++) {
			String val = attributes.getValue(i).trim();
			switch (SVGAttr.fromString(attributes.getLocalName(i))) {
	 ...
	 */
	private void parseAttributesPolyLine(SVG.PolyLine obj,
			Attributes attributes, String tag) throws SAXException {
		for (int i = 0; i < attributes.getLength(); i++) {
	...
	private void polygon(Attributes attributes) throws SAXException {
		...
		parseAttributesTransform(obj, attributes);
		parseAttributesConditional(obj, attributes);
		parseAttributesPolyLine(obj, attributes, "polygon"); // reuse of
																// polyline
																// "points"
																// parser
		currentElement.addChild(obj);
	...
	private void parseAttributesTextPosition(TextPositionedContainer obj,
			Attributes attributes) throws SAXException {
		for (int i = 0; i < attributes.getLength(); i++) {
			String val = attributes.getValue(i).trim();
			switch (SVGAttr.fromString(attributes.getLocalName(i))) {
	...
	private void parseAttributesConditional(SVG.SvgConditional obj,
			Attributes attributes) throws SAXException {
		for (int i = 0; i < attributes.getLength(); i++) {
			String val = attributes.getValue(i).trim();
			switch (SVGAttr.fromString(attributes.getLocalName(i))) {
	...
	private void parseAttributesMarker(SVG.Marker obj, Attributes attributes)
			throws SAXException {
		for (int i = 0; i < attributes.getLength(); i++) {
			String val = attributes.getValue(i).trim();
			switch (SVGAttr.fromString(attributes.getLocalName(i))) {
			...
			case markerUnits:
				if ("strokeWidth".equals(val)) {
					...
					throw new SAXException(
							"Invalid value for attribute markerUnits");
				}
				break;
			case orient:
				if ("auto".equals(val)) {
	...
	private void parseAttributesGradient(SVG.GradientElement obj,
			Attributes attributes) throws SAXException {
		for (int i = 0; i < attributes.getLength(); i++) {
			String val = attributes.getValue(i).trim();
			switch (SVGAttr.fromString(attributes.getLocalName(i))) {
			case gradientUnits:
				if ("objectBoundingBox".equals(val)) {
					...
					throw new SAXException(
							"Invalid value for attribute gradientUnits");
				}
				break;
	...
	private void parseAttributesLinearGradient(SVG.SvgLinearGradient obj,
			Attributes attributes) throws SAXException {
		for (int i = 0; i < attributes.getLength(); i++) {
			String val = attributes.getValue(i).trim();
			switch (SVGAttr.fromString(attributes.getLocalName(i))) {
	...
	private void parseAttributesRadialGradient(SVG.SvgRadialGradient obj,
			Attributes attributes) throws SAXException {
		for (int i = 0; i < attributes.getLength(); i++) {
			String val = attributes.getValue(i).trim();
			switch (SVGAttr.fromString(attributes.getLocalName(i))) {
	...
	private Float parseGradiantOffset(String val) throws SAXException {
		if (val.length() == 0)
			throw new SAXException(
					"Invalid offset value in <stop> (empty string)");
		int end = val.length();
		...
		for (int i = 0; i < attributes.getLength(); i++) {
			...
			case clipPathUnits:
				if ("objectBoundingBox".equals(val)) {
					...
					throw new SAXException(
							"Invalid value for attribute clipPathUnits");
				}
				break;
			default:
				break;
		...
		for (int i = 0; i < attributes.getLength(); i++) {
			...
			case patternUnits:
				if ("objectBoundingBox".equals(val)) {
					...
					throw new SAXException(
							"Invalid value for attribute patternUnits");
				}
				break;
			case patternContentUnits:
				if ("objectBoundingBox".equals(val)) {
					...
					throw new SAXException(
							"Invalid value for attribute patternContentUnits");
				}
				break;
	...
	private void parseAttributesMask(SVG.Mask obj, Attributes attributes)
			throws SAXException {
		for (int i = 0; i < attributes.getLength(); i++) {
			String val = attributes.getValue(i).trim();
			switch (SVGAttr.fromString(attributes.getLocalName(i))) {
			case maskUnits:
				if ("objectBoundingBox".equals(val)) {
					...
					throw new SAXException(
							"Invalid value for attribute maskUnits");
				}
				break;
			case maskContentUnits:
				if ("objectBoundingBox".equals(val)) {
					...
					throw new SAXException(
							"Invalid value for attribute maskContentUnits");
				}
				break;
			case x:
				obj.x = parseLength(val);
	...
	protected static class TextScanner {
		...
		public Float nextFloat() {
			NumberParser np = NumberParser.parseNumber(input, position,
					inputLength);

			// System.out.println("SVGParser.TextScanner.nextFloat()"+input);

			if (np == null)
				return null;
			position = np.getEndPos();

			Log.d("Np.value", "" + np.value());

			// MainActivity.coords1.add(np.value());

			return np.value();
		}

		/*
		 * Scans for a comma-whitespace sequence with a float following it. If
		 * found, the float is returned. Otherwise null is returned and the scan
		...
		/*
		 * Scans for comma-whitespace sequence with a float following it. But
		 * only if the provided 'lastFloat' (representing the last coord scanned
		 * was non-null (ie parsed correctly).
		 */
		...
		public Integer nextInteger() {
			IntegerParser ip = IntegerParser.parseInt(input, position,
					inputLength);
			if (ip == null)
				return null;
		...
		/*
		 * Scans the input starting immediately at 'position' for the next
		 * token. A token is a sequence of characters terminating at a
		 * whitespace character. Note that this routine only checks for
		 * whitespace characters. Use nextToken(char) if token might end with
		 * another character.
		 */
		...
		/*
		 * Scans the input starting immediately at 'position' for the next
		 * token. A token is a sequence of characters terminating at either a
		 * whitespace character or the supplied terminating character.
		 */
		 ...
		 * of letter characters terminated by an open bracket. The function name
		 * is returned.
		 */
		...
		public Unit nextUnit() {
			...
			if (position > (inputLength - 2))
				return null;
			try {
				Unit result = Unit.valueOf(input.substring(position,
						position + 2).toLowerCase(Locale.US));
				position += 2;
				return result;
			} catch (IllegalArgumentException e) {
				return null;
	...
	private void parseAttributesCore(SvgElementBase obj, Attributes attributes)
		...
		for (int i = 0; i < attributes.getLength(); i++) {
			...
			// System.out.println("SVGParser.parseAttributesCore()"+qname);
			...
			if (qname.equals("id") || qname.equals("xml:id")) {
				...
				if ("default".equals(val)) {
					...
					throw new SAXException(
							"Invalid value for \"xml:space\" attribute: " + val);
				}
				break;
			}
		}
	...
	private void parseAttributesStyle(SvgElementBase obj, Attributes attributes)
		...
		for (int i = 0; i < attributes.getLength(); i++) {
			...
			if (attributes.getLocalName(i).equalsIgnoreCase("style")) {
				Log.w("Attribute of path",
						String.valueOf(attributes.getValue(i)));// Its to print
																// the style if
																// it is there
																// in PATH
			...
			case style:
				parseStyle(obj, val);
				// System.out.println("SVGParser.parseAttributesStyle()"+obj.classNames);
				break;
	...
	private static void parseStyle(SvgElementBase obj, String style)
			throws SAXException {
		// System.out.println("SVGParser.parseStyle()"+style);
		...
		// Log.w("parsingStyle", style.replaceAll("/\\*.*?\\*/", ""));
		while (true) {
	...
	protected static void processStyleProperty(Style style, String localName,
		...
		case display:
			if (val.indexOf('|') >= 0
					|| (VALID_DISPLAY_VALUES.indexOf('|' + val + '|') == -1))
				throw new SAXException(
		...
		case visibility:
			if (val.indexOf('|') >= 0
					|| (VALID_VISIBILITY_VALUES.indexOf('|' + val + '|') == -1))
				throw new SAXException(
	...
	private void parseAttributesViewBox(SVG.SvgViewBoxContainer obj,
			Attributes attributes) throws SAXException {
		for (int i = 0; i < attributes.getLength(); i++) {
			String val = attributes.getValue(i).trim();
			switch (SVGAttr.fromString(attributes.getLocalName(i))) {
			case viewBox:
				...
				// Log.d("valueOfWidthaAndHeight", val);

				obj.viewBox = parseViewBox(val);
				break;
			case preserveAspectRatio:
				// Log.d("valueOfWidthaAndHeight(AspectRatio)", val);
				parsePreserveAspectRatio(obj, val);
				break;
			default:
				break;
	...
	private void parseAttributesTransform(SVG.HasTransform obj,
			Attributes attributes) throws SAXException {
		for (int i = 0; i < attributes.getLength(); i++) {
	...
	private Matrix parseTransformList(String val) throws SAXException {
		...
		while (!scan.empty()) {
			...
			if (cmd == null)
				throw new SAXException(
						"Bad transform function encountered in transform list: "
								+ val);
	...
	/*
	 * Parse an SVG 'Length' value (usually a coordinate). Spec says: length ::=
	...
	protected static Length parseLength(String val) throws SAXException {
		...
		if (lastChar == '%') {
			...
			unit = Unit.percent;
		} else if (end > 2 && Character.isLetter(lastChar)
				&& Character.isLetter(val.charAt(end - 2))) {
			end -= 2;
	...
	private static List<Length> parseLengthList(String val) throws SAXException {
		...
		while (!scan.empty()) {
			...
			if (scalar == null)
				throw new SAXException("Invalid length list value: "
						+ scan.ahead());
	...
	private static float parseFloat(String val, int offset, int len)
			throws SAXException {
    ...
    * 
    */
	private static void parsePreserveAspectRatio(
			SVG.SvgPreserveAspectRatioContainer obj, String val)
			throws SAXException {
		...
		if (!scan.empty()) {
			...
			if (meetOrSlice.equals("meet")) {
				...
				throw new SAXException(
						"Invalid preserveAspectRatio definition: " + val);
			}
		}
	...
	private static synchronized void initialiseAspectRatioKeywordsMap() {
		aspectRatioKeywords = new HashMap<String, PreserveAspectRatio.Alignment>(
				10);
		aspectRatioKeywords.put(NONE, PreserveAspectRatio.Alignment.None);
		aspectRatioKeywords.put("xMinYMin",
				PreserveAspectRatio.Alignment.XMinYMin);
		aspectRatioKeywords.put("xMidYMin",
				PreserveAspectRatio.Alignment.XMidYMin);
		aspectRatioKeywords.put("xMaxYMin",
				PreserveAspectRatio.Alignment.XMaxYMin);
		aspectRatioKeywords.put("xMinYMid",
				PreserveAspectRatio.Alignment.XMinYMid);
		aspectRatioKeywords.put("xMidYMid",
				PreserveAspectRatio.Alignment.XMidYMid);
		aspectRatioKeywords.put("xMaxYMid",
				PreserveAspectRatio.Alignment.XMaxYMid);
		aspectRatioKeywords.put("xMinYMax",
				PreserveAspectRatio.Alignment.XMinYMax);
		aspectRatioKeywords.put("xMidYMax",
				PreserveAspectRatio.Alignment.XMidYMax);
		aspectRatioKeywords.put("xMaxYMax",
				PreserveAspectRatio.Alignment.XMaxYMax);
	}
	...
	private static Colour parseColour(String val) throws SAXException {
		if (val.charAt(0) == '#') {
			...
			if (pos == 7) {
				...
				return new Colour(h1 << 16 | h1 << 12 | h2 << 8 | h2 << 4
						| h3 << 4 | h3);
			}
	...
	private static int parseColourComponent(TextScanner scan)
			throws SAXException {
		...
		// CSS rules say that only percent values should be clamped, but we will
		// do it for both.
	...
	private static Style.LineCaps parseStrokeLineCap(String val)
			throws SAXException {
		if ("butt".equals(val))
	...
	private static Length[] parseStrokeDashArray(String val)
		...
		if (dash.isNegative())
			throw new SAXException(
					"Invalid stroke-dasharray. Dash segemnts cannot be negative: "
							+ val);
		...
		while (!scan.empty()) {
			...
			if (dash == null) // must have hit something unexpected
				throw new SAXException(
						"Invalid stroke-dasharray. Non-Length content found: "
								+ val);
			if (dash.isNegative())
				throw new SAXException(
						"Invalid stroke-dasharray. Dash segemnts cannot be negative: "
		...
		// Spec (section 11.4) says if the sum of dash lengths is zero, it
		// should
	...
	private static SVG.PathDefinition parsePath(String val) throws SAXException {
		...
		Log.d("Parsing Path", val);

		TextScanner scan = new TextScanner(val);

		int pathCommand = '?';
		float currentX = 0f, currentY = 0f; // The last point visited in the
											// subpath
		float lastMoveX = 0f, lastMoveY = 0f; // The initial point of current
												// subpath
		float lastControlX = 0f, lastControlY = 0f; // Last control point of the
													// just completed bezier
													// curve.
		...
		// System.out.println("SVGParser.parsePath()"+pathCommand);
		...
		while (true) {
			...
			case 'm':
				...
				if (y == null) {
					Log.e(TAG, "Bad path coords for " + ((char) pathCommand)
							+ " path segment");
					return path;
				}
				// Relative moveto at the start of a path is treated as an
				// absolute moveto.
			...
			// Test to see if there is another set of coords for the current
			// path command
			if (scan.hasLetter()) {
	...
	private static Set<String> parseRequiredFeatures(String val)
		...
		while (!scan.empty()) {
			...
			if (feature.startsWith(FEATURE_STRING_PREFIX)) {
				...
				// Not a feature string we recognise or support. (In order to
				// avoid accidentally
				// matches with our truncated feature strings, we'll replace it
				// with a string
	...
	private static Set<String> parseSystemLanguage(String val)
		...
		while (!scan.empty()) {
			...
			// Get canonical version of language code in case it has changed
			// (see the JavaDoc for Locale.getLanguage())
	...
	private static String parseFunctionalIRI(String val, String attrName)
		...
		// Unlike CSS, the SVG spec seems to indicate that quotes are not
		// allowed in "url()" references
	}
