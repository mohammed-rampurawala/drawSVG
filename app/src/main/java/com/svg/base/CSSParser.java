...
import com.svg.base.SVGParser.TextScanner;
 ...
 * The main goals are to (a) be small, and (b) parse the CSS in a Corel Draw SVG
 * file.
 * 
...
public class CSSParser {
	...
	private static class SimpleSelector {
		...
		public SimpleSelector(Combinator combinator, String tag) {
			this.combinator = (combinator != null) ? combinator
	...
	public static class Ruleset {
		...
		// Add a rule to the ruleset. The position at which it is inserted is
		// determined by its specificity value.
	...
	public static class Selector {
		...
		// Methods for accumulating a specificity value as SimpleSelector
		// entries are added.
	...
	public static boolean mediaMatches(String mediaListStr,
			MediaType rendererMediaType) throws SAXException {
	...
	/*
	 * private static void error(String format, Object... args) { Log.e(TAG,
	 * String.format(format, args)); }
	 * 
	 * 
	 * private static void debug(String format, Object... args) { if
	 * (LibConfig.DEBUG) Log.d(TAG, String.format(format, args)); }
	 */
	...
	private static class CSSTextScanner extends TextScanner {
		public CSSTextScanner(String input) {
			super(input.replaceAll("(?s)/\\*.*?\\*/", "")); // strip all block
															// comments
		}
		...
		/*
		 * Scans for a CSS 'simple selector'. Returns true if it found one.
		...
		public boolean nextSimpleSelector(Selector selector)
			...
			while (!empty()) {
				if (consume('.')) {
					...
					if (value == null)
						throw new SAXException(
								"Invalid \".class\" selector in <style> element");
				...
				if (consume('#')) {
					...
					if (value == null)
						throw new SAXException(
								"Invalid \"#id\" selector in <style> element");
				...
				if (consume('[')) {
					...
					if (attrName == null)
						throw new SAXException(
								"Invalid attribute selector in <style> element");
					skipWhitespace();
					AttribOp op = null;
					if (consume('='))
						op = AttribOp.EQUALS;
					...
					if (!consume(']'))
						throw new SAXException(
								"Invalid attribute selector in <style> element");
					selectorPart.addAttrib(attrName,
							(op == null) ? AttribOp.EXISTS : op, attrValue);
					selector.addedAttributeOrPseudo();
					continue;
				}
				...
				if (consume(':')) {
					...
					if (nextIdentifier() != null) {
						...
						selectorPart.addPseudo(input.substring(pseudoStart,
								position));
						selector.addedAttributeOrPseudo();
					}
				}
	...
	// Returns true if 'rendererMediaType' matches one of the media types in
	// 'mediaList'
	private static boolean mediaMatches(List<MediaType> mediaList,
			MediaType rendererMediaType) {
	...
	private void parseAtRule(Ruleset ruleset, CSSTextScanner scan)
			throws SAXException {
		...
		if (!inMediaRule && atKeyword.equals("media")) {
			...
			if (!scan.consume('}'))
				throw new SAXException(
						"Invalid @media rule: expected '}' at end of rule set");
	...
	// Skip an unsupported at-rule:
	// "ignore everything up to and including the next semicolon or block".
	...
	private boolean parseRule(Ruleset ruleset, CSSTextScanner scan)
			throws SAXException {
		List<Selector> selectors = parseSelectorGroup(scan);
		if (selectors != null && !selectors.isEmpty()) {
			if (!scan.consume('{'))
				throw new SAXException(
						"Malformed rule block in <style> element: missing '{'");
			scan.skipWhitespace();
	...
	/*
	 * Parse a selector group (eg. E, F, G). In many/most cases there will be
	 * only one entry.
	 */
	private List<Selector> parseSelectorGroup(CSSTextScanner scan)
			throws SAXException {
		if (scan.empty())
			return null;
		...
		while (!scan.empty()) {
			if (scan.nextSimpleSelector(selector)) {
				// If there is a comma, keep looping, otherwise break
				if (!scan.skipCommaWhitespace())
					continue; // if not a comma, go back and check for next part
								// of selector
	...
	// Parse a list of
	private SVG.Style parseDeclarations(CSSTextScanner scan)
			throws SAXException {
		...
		while (true) {
			...
			scan.consume(';');
			SVGParser.processStyleProperty(ruleStyle, propertyName,
					propertyValue);
			scan.skipWhitespace();
			if (scan.consume('}'))
				return ruleStyle;
	 ...
	 */
	protected static List<String> parseClassAttribute(String val)
			throws SAXException {
	...
	protected static boolean ruleMatch(Selector selector, SvgElementBase obj) {
		...
		// We start at the last part of the selector and loop back through the
		// parts
		// Get the next selector part
		return ruleMatch(selector, selector.size() - 1, ancestors,
				ancestorsPos, obj);
	}
	...
	private static boolean ruleMatch(Selector selector, int selPartPos,
		...
		if (sel.combinator == Combinator.DESCENDANT) {
			if (selPartPos == 0)
				return true;
			// Search up the ancestors list for a node that matches the next
			// selector
			while (ancestorsPos >= 0) {
				if (ruleMatchOnAncestors(selector, selPartPos - 1, ancestors,
						ancestorsPos))
					return true;
				ancestorsPos--;
			}
			...
			return ruleMatchOnAncestors(selector, selPartPos - 1, ancestors,
					ancestorsPos);
		} else // if (sel.combinator == Combinator.FOLLOWS)
		{
			...
			if (childPos <= 0)
				return false;
			SvgElementBase prevSibling = (SvgElementBase) obj.parent
					.getChildren().get(childPos - 1);
			return ruleMatch(selector, selPartPos - 1, ancestors, ancestorsPos,
					prevSibling);
		}
	}

	private static boolean ruleMatchOnAncestors(Selector selector,
			int selPartPos, List<SvgContainer> ancestors, int ancestorsPos) {
		SimpleSelector sel = selector.get(selPartPos);
	...
	private static int getChildPosition(List<SvgContainer> ancestors,
	...
	private static boolean selectorMatch(SimpleSelector sel,
			List<SvgContainer> ancestors, int ancestorsPos, SvgElementBase obj) {
		// Check tag name. tag==null means tag is "*" which matches everything.
		if (sel.tag != null) {
			// The Group object does not match its tag ("<g>"), so we have to
			...
			else if (!sel.tag.equals(obj.getClass().getSimpleName()
