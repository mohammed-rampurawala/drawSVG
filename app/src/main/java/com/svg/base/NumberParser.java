 ...
 * 
 * We use our own parser because the one in Android (from Harmony I think) is
 * slow.
 * 
 * An SVG 'number' is defined as integer ([Ee] integer)? | [+-]? [0-9]* "."
 * [0-9]+ ([Ee] integer)? Where 'integer' is [+-]? [0-9]+ CSS numbers were
 * different, but have now been updated to a compatible definition (see 2.1
 * Errata) [+-]?([0-9]+|[0-9]*\.[0-9]+)(e[+-]?[0-9]+)?
 * 
...
public class NumberParser {
	...
	public NumberParser(boolean isNegative, long significand, int exponent,
			int pos) {
	...
	/*
	 * Scan the string for an SVG number. Assumes maxPos will not be greater
	 * than str.length().
	 */
	public static NumberParser parseNumber(String input, int startpos, int len) {
		...
		// Log.d("char in number parser", String.valueOf(ch));
		...
		while (pos < len) {
			...
			if (ch == '0') {
				if (numDigits == 0) {
					...
					// System.out.println("NumberParser.parseNumber() Number of leading zeroes"+numLeadingZeroes);
				} else {
					// We potentially skip trailing zeroes. Keep count for now.
					numTrailingZeroes++;

					// System.out.println("NumberParser.parseNumber() Number of trailing zeroes"+numTrailingZeroes);
				}
			} else if (ch >= '1' && ch <= '9') {
				// Log.d("char in number parser", String.valueOf(ch));
				...
				numDigits = numDigits + numTrailingZeroes;
		...
		if (significand == 0) {
			...
			// Log.d("In Last if of Number Parser (True)",String.valueOf(isNegative));

			// Log.d("In Last if of Number Parser(True)",String.valueOf(pos));

			return new NumberParser(isNegative, 0, 0, pos);

		} else {

			// Log.d("In Last if of Number Parser (False)",String.valueOf(isNegative));

			// Log.d("In Last if of Number Parser (False)",String.valueOf(significand));

			// Log.d("In Last if of Number Parser (False)",String.valueOf(exponent));

			// Log.d("In Last if of Number Parser (False)",String.valueOf(pos));

			return new NumberParser(isNegative, significand, exponent, pos);
		}
	}

	private static final float positivePowersOf10[] = { 1e0f, 1e1f, 1e2f, 1e3f,
			1e4f, 1e5f, 1e6f, 1e7f, 1e8f, 1e9f, 1e10f, 1e11f, 1e12f, 1e13f,
			1e14f, 1e15f, 1e16f, 1e17f, 1e18f, 1e19f, 1e20f, 1e21f, 1e22f,
			1e23f, 1e24f, 1e25f, 1e26f, 1e27f, 1e28f, 1e29f, 1e30f, 1e31f,
			1e32f, 1e33f, 1e34f, 1e35f, 1e36f, 1e37f, 1e38f };
	private static final float negativePowersOf10[] = { 1e0f, 1e-1f, 1e-2f,
			1e-3f, 1e-4f, 1e-5f, 1e-6f, 1e-7f, 1e-8f, 1e-9f, 1e-10f, 1e-11f,
			1e-12f, 1e-13f, 1e-14f, 1e-15f, 1e-16f, 1e-17f, 1e-18f, 1e-19f,
			1e-20f, 1e-21f, 1e-22f, 1e-23f, 1e-24f, 1e-25f, 1e-26f, 1e-27f,
			1e-28f, 1e-29f, 1e-30f, 1e-31f, 1e-32f, 1e-33f, 1e-34f, 1e-35f,
			1e-36f, 1e-37f, 1e-38f };
	...
	public float value() {
		...
		if (exponent > 0) {
			...
			// Some valid numbers can have an exponent greater than the max (ie.
			// < -38)
			// for a float. For example, significand=123, exponent=-40
			// If that's the case, we need to apply the exponent in two steps.
			if (exponent < -38) {
				// Long.MAX_VALUE is 19 digits, so taking 20 off the exponent
				// should be enough.
				f *= 1e-20;
				exponent += 20;
			}
