...
public class SVGImageView extends ImageView {
	...
	public static SVG mySvgObject = null;

	private static Method setLayerTypeMethod = null;

	{
		try {
			setLayerTypeMethod = View.class.getMethod("setLayerType",
	...
	private boolean internalSetImageURI(Uri uri, boolean isDirectRequestFromUser) {
		...
		try {
			...
			Log.w("Internal image URI", uri.toString());
	...
	private class LoadResourceTask extends AsyncTask<Integer, Integer, Picture> {
		protected Picture doInBackground(Integer... resourceId) {
			try {
				...
				Log.e("SVGImageView", String.format(
						"Error loading resource 0x%x: %s", resourceId,
						e.getMessage()));
			}
			return null;
		}
	...
	private class LoadAssetTask extends AsyncTask<String, Integer, Picture> {
		protected Picture doInBackground(String... filename) {
			try {
				SVG svg = SVG.getFromAsset(getContext().getAssets(),
						filename[0]);

				return svg.renderToPicture();
			} catch (SVGParseException e) {
				Log.e("SVGImageView", "Error loading file " + filename + ": "
						+ e.getMessage());
			} catch (FileNotFoundException e) {
				Log.e("SVGImageView", "File not found: " + filename);
			} catch (IOException e) {
				Log.e("SVGImageView", "Unable to load asset file: " + filename,
						e);
			}
			return null;
		}
	...
	private class LoadURITask extends AsyncTask<InputStream, Integer, Picture> {
		protected Picture doInBackground(InputStream... is) {
			try {
				...
				Log.w("in LOADURITASK", "Loading URI");

				SVG svg = SVG.getFromInputStream(is[0]);
				return svg.renderToPicture();
			} catch (SVGParseException e) {
				Log.e("SVGImageView",
						"Parse error loading URI: " + e.getMessage());
			} finally {
				try {
					is[0].close();
	...
	private void setSoftwareLayerType() {
		...
		try {
			int LAYER_TYPE_SOFTWARE = View.class
					.getField("LAYER_TYPE_SOFTWARE").getInt(
							new View(getContext()));
